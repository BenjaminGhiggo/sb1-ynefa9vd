...
import SubirArchivoFacturaImagenDocumento from './SubirArchivoFacturaImagenDocumento';

interface Props {
  onVolver: () => void;
}

const SubirArchivoFactura: React.FC<Props> = ({ onVolver }) => {
  const [verOpciones, setVerOpciones] = useState(false);

  if (verOpciones) {
    return <SubirArchivoFacturaImagenDocumento onVolver={() => setVerOpciones(false)} />;
  }

  return (
    <div className="max-w-md mx-auto text-center pb-24">
      <h2 className="text-xl font-bold text-blue-800 mb-6">Subir archivo factura</h2>
...
export default SubirArchivoFactura;
